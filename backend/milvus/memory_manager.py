...
_LangMemBase = declarative_base()


class _MemoryRecord(_LangMemBase):
...
def _init_memory_table(engine):
    ...
    try:
        _LangMemBase.metadata.create_all(bind=engine)
...
class _SQLAlchemyStore:
    ...
    def put(self, namespace: tuple, key: str, value: dict) -> None:
        ...
        try:
            record = (
                db.query(_MemoryRecord)
                .filter(_MemoryRecord.namespace == ns, _MemoryRecord.key == key)
                .first()
            )
            if record:
    ...
    def delete(self, namespace: tuple, key: str) -> None:
        ...
        db = SessionLocal()
        try:
            db.query(_MemoryRecord).filter(
                _MemoryRecord.namespace == ns, _MemoryRecord.key == key
    ...
    def search(
        ...
        **kwargs: Any,
        ...
        try:
            records = (
                db.query(_MemoryRecord)
                .filter(_MemoryRecord.namespace.like(f"{ns}%"))
                .limit(limit)
                .all()
            )
            results = []
    ...
    def list_namespaces(self, prefix: tuple = (), max_depth: int = 1) -> List[tuple]:
        ...
        try:
            records = (
                db.query(_MemoryRecord.namespace)
                .filter(_MemoryRecord.namespace.like(f"{ns_prefix}%"))
                .distinct()
                .all()
            )
