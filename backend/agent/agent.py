...
def _build_system_prompt(user_memories: str) -> str:
    ...
    base_prompt = (
        ...
        "2. Call search_knowledge_base to retrieve; use top_k to control recall count\n"
        "3. The system automatically evaluates document relevance and rewrites query if needed\n"
        "4. After retrieval, judge if results are sufficient to answer:\n"
        "   - If sufficient: answer directly\n"
        "   - If you need different perspectives: call again with a different query angle\n"
        "   - Maximum 3 calls per turn; if still insufficient: honestly state inability to answer\n"
