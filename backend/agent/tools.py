...
@tool("search_entity_relationship")
def search_entity_relationship(
    query: str = "",
    entity_a: str = "",
    entity_b: str = "",
    max_hops: int = 3,
    ...
    适用场景：
    - 用户询问两个实体之间的关系（提供 entity_a 和 entity_b）
    - 用户询问某个实体相关的其他实体（只提供 entity_a）
    - 用户询问概念之间的连接

    Args:
        query: 自然语言问题（如果未提供 entity_a/b，则从中抽取实体）
        entity_a: 第一个实体名称（必填）
        entity_b: 第二个实体名称（可选；提供时查询两实体间路径）
        max_hops: 最大跳数（默认 3，最大 5）
    """
    ...
    try:
        ...
        if not retriever.neo4j.is_connected():
            return "知识图谱未连接，无法查询实体关系。请使用 search_knowledge_base 搜索。"

        # 如果未提供实体，尝试从 query 中抽取
        if not entity_a and query:
            ...
            if len(entities) >= 2:
                ...
                return (
                    "未能从问题中识别出实体。请直接指定实体名称，"
                    '例如：entity_a="Python", entity_b="Django"'
                )

        if not entity_a:
            return "请提供至少一个实体名称 (entity_a)。"

        results = []

        # 两实体路径查询
        if entity_b:
            paths = retriever.multi_hop_query(entity_a, entity_b, max_hops=max_hops)
            if paths:
                results.append(f"## {entity_a} 与 {entity_b} 的关系路径：\n")
                for i, path in enumerate(paths, 1):
                    results.append(f"路径 {i}: {path['path_description']}")
                    results.append(f"  涉及实体: {' → '.join(path['entities'])}")
                    results.append(f"  关系链: {' → '.join(path['relations'])}")
                    results.append("")
            else:
                results.append(
                    f"未找到 {entity_a} 与 {entity_b} 之间的关系路径（最多 {max_hops} 跳）。"
                )

        # 单实体直接关联
        neighbors = retriever.get_related_entities(entity_a)
        if neighbors:
            results.append(f"\n## 与 {entity_a} 直接相关的实体：\n")
            for n in neighbors[:10]:
                relation = n.get("relation", "未知关系")
                confidence = n.get("confidence", 0)
                ntype = n.get("type", "")
                desc = n.get("description", "")
                line = f"- {n['name']} ({ntype}): {relation}"
                if confidence:
        ...
        if not results:
            return f"未找到与 {entity_a} 相关的实体或关系。"

        return "\n".join(results)

    except Exception as e:
        logger.warning(f"实体关系查询失败: {e}")
        return f"实体关系查询出错: {e}。请尝试使用 search_knowledge_base 搜索。"
